...
struct DarkDBChunkPLAYERCAM {
    uint32 attachment; // 5 before SIM, 0 after, 3 or 4 in scouting orb mode, 2
                       // when attached to an object
    float
        fov; // 1.0 is normal. >1 contracts fov (zoom in), <1 expands (fisheye)
    Coord position;
    SCoord rotation;
...
struct DarkDBRoom {
    ...
    // (The IDs should be in both lists, unless you know of an AI that isn't an
    // object.)
};
#pragma pack(pop)
 ...
 * WR/WRRGB		// [MIS] [1]/[2,S] WorldRep - Understand this and you can
 * do away with Dromed
 */
