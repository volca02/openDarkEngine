...
public:
    ...
    virtual bool isEnumerated() { return false; }

    virtual DEnumPtr getEnum() { return DEnumPtr(NULL); }

    virtual DVariant::Type type() { return DVariant::DV_INVALID; }
...
/** Reference counted dynamic type definition. Used to create a descriptive
 * object, which can be used to access structures/arrays/unions in memory. The
 * structure of DTypeDef can only be modified at creation time, that means that
 * no structural changes are possible once the dyntype is constructed.
 *
...
public:
    ...
    /// Constucts a possibly enumerated (if _enum member is not NULL) type with
    /// default value and type taken from templ
    DTypeDef(const std::string &name, const DVariant &templ, size_t size,
             const DEnumPtr &_enum);

    /// Constucts a possibly enumerated (if _enum is not NULL) type with type
    /// taken from templ
    DTypeDef(const std::string &name, DVariant::Type type, size_t size,
             const DEnumPtr &_enum);

    /** Construct an array type, with the type defined by member, and default
     * value of the fields defined by member, sized size
     * @note The base name is taken from the member
     */
    DTypeDef(const DTypeDefPtr &member, size_t size);

    /// Construct a Structure with members defined by the list. If unioned is
    /// true, all the fields share the same start offset (0)
    DTypeDef(const std::string &name, DTypeDefVector members,
             bool unioned = false);
    ...
    /** Returns true if the instance is a struct - multiple childs, each can
    ...
    /** Create an alias to this DTypeDef, assigning a new name. Functionally
     * equal to DTypeDef(name, type_to_clone); The purpose of this is to support
     * struct field names and aliased structs
     * @param name New name for this typedef
     * @note Setting the default value must be done after aliasing, as this also
     * aliases container types
     */
    ...
    /** Initializes the given data pointer (has to be allocated to the length of
     * at least size()) */
    void toDefaults(char *data);

    /** Get the value of the field specified by name, reading the data from the
     * dataptr mem. location
     * @note The field name has to exist, and has to be a simple field (Not
     * array/union/struct field)
     * @note No check on the dataptr allocated size is done. This means the
     * allocated size has to be at least the same as the size of this dtypedef
     */
    ...
    inline const DVariant &getDefault() { return mDefVal; }

    /// Returns true if this field has any default value
    inline bool hasDefault() { return mDefaultUsed; }
    ...
    struct FieldDef {
        ...
        inline void setDefault(char *dataptr) const {
            type->_set(reinterpret_cast<char *>(dataptr) + offset,
                       type->getDefault());
        }
    ...
    /// Field list. Order is sequential (increasing offset), with one exception
    /// - unions
    ...
    /// The direct field access cache. This has to be refreshed manually after
    /// definition is complete
    typedef std::map<std::string, FieldDef> FieldMap;

    inline const_iterator begin() { return mFields.begin(); }

    inline const_iterator end() { return mFields.end(); }

    inline const std::string &name() { return mTypeName; }
...
protected:
    ...
    /// Default value. Only valid if the dtypedef is simple. This fills the
    /// fields of a newly created type
    DVariant mDefVal;
...
/** DTypeDef 'variable'. Ugly and a bit outdated concept that should be got rid
 * off soon.
 * @deprecated */
class OPDELIB_EXPORT DType {
public:
    /** Copy constructor. Copies the data and the type definition from another
     * DType instance */
    DType(const DType &b, bool useCache = false)
        : mUseCache(useCache), mCache() {
        char *odata = mData;
    ...
    /** Empty constructor. Constructs a new type instance using the default
     * values. */
    DType(const DTypeDefPtr &type, bool useCache = false)
        : mType(type), mUseCache(useCache) {
     ...
     * @note for dynamic size types (variable length strings), the size should
     * be the overall length of the data (32bits size + the data itself) */
    DType(const DTypeDefPtr &type, FilePtr &file, size_t _size,
          bool useCache = false)
        : mType(type), mUseCache(useCache) {
        mData = 0;
        read(file, _size);
    ...
    size_t size() const { return mType->size(); }

    /** Serializer. Writes the type data into a FilePtr
     * @param file The file pointer to write into */
    void serialize(FilePtr &file) const { file->write(mData, size()); }
...
/// Data field const iterator. The const is ommited to leave the type's name in
/// reasonable length
...
class OPDELIB_EXPORT EmptyDataFieldDescListIterator
    : public DataFieldDescListIterator {
public:
