...
void SaveMaterialFile(const char *basename, const char *path, BinHeader &hdr) {
    ...
    for (n = 0; n < hdr.num_mats; n++) {
        ...
        if (materials[n].type == MD_MAT_COLOR) {
            ...
            out << "\t\t\tspecular " << illum << " " << illum << " " << illum
                << endl;
            // enda line
            out << endl;
...
void addTriangle(BinHeader &hdr, SubObjectHeader &shdr, int objidx,
                 unsigned char material, short ida, short idb, short idc,
                 short uva, short uvb, short uvc) {

    log_verbose("\t\t\t\tAdding triangle : material %d, vertex indices %d, %d, "
                "%d (UV: %d, %d, %d)",
                material, ida, idb, idc, uva, uvb, uvc);
...
void addTriangle(BinHeader &hdr, SubObjectHeader &shdr, int objidx,
    ...
    log_verbose("\t\t\t\tAdding no-tex triangle : material %d, vertex indices "
                "%d, %d, %d",
                material, ida, idb, idc);
...
void LoadDirectTriList(ifstream &in, BinHeadType &thdr, BinHeader &hdr,
                       BinHeader2 &hdr2, int objidx, SubObjectHeader &shdr) {
    ...
    // I don't quite understand where to get the material number for this
    // version of Mesh maybe there is a list of materials per triangle after the
    // parts offset, we'll see
    int n;

    for (n = 0; n < num_uvs; n++)
        addTriangle(hdr, shdr, objidx, 0, parts[n].a, parts[n].b, parts[n].c,
                    parts[n].a, parts[n].b, parts[n].c);

    delete[] parts;
}

void loadPolygon(ifstream &in, BinHeadType &thdr, BinHeader &hdr,
                 BinHeader2 &hdr2, int objidx, SubObjectHeader &shdr,
                 long offset) {
    log_verbose("\t\t\t\tloading polygon on offset %04X : ",
                hdr.offset_pgons + offset);
    int oldpos = in.tellg();
    ...
    if (thdr.version == 4)
        ...
        int mat = SlotToMatIndex(
            polyHdr.data); // Material here uses a slot... we'll se if the v4
                           // does that too (so no -1)
        log_verbose("\t\t\t\tMaterial, type, textured : %d %02X %d", mat,
                    polyHdr.type, (polyHdr.type == MD_PGON_TMAP));

        if ((mat < 0) || (mat >= hdr.num_mats)) {
            log_error("\t\t\t\tInvalid material number %d, not adding the "
                      "polygon (slot %d)",
                      mat, polyHdr.data);
            return;
        }
    ...
    for (int i = 0; i < polyHdr.num_verts - 1; i++) {
        ...
        if (polyHdr.type == MD_PGON_TMAP)
            addTriangle(hdr, shdr, objidx, Material, vertex_indices[0],
                        vertex_indices[i + 1], vertex_indices[i], uv_indices[0],
                        uv_indices[i + 1], uv_indices[i]);
        else {
            if (materials[static_cast<size_t>(Material)].type == MD_MAT_TMAP) {
                log_error("Material needs UV and none found, using vertex "
                          "index for uv");
                addTriangle(hdr, shdr, objidx, Material, vertex_indices[0],
                            vertex_indices[i + 1], vertex_indices[i],
                            vertex_indices[0], vertex_indices[i + 1],
                            vertex_indices[i]);
            } else
                addTriangle(hdr, shdr, objidx, Material, vertex_indices[0],
                            vertex_indices[i + 1], vertex_indices[i]);
...
void parseSubNode(ifstream &in, BinHeadType &thdr, BinHeader &hdr,
                  BinHeader2 &hdr2, int objidx, SubObjectHeader &shdr,
                  long offset) {
    char splittype;
    short polys[1024];
    ...
    log_debug("\t\t\tSub-node %d on offset %04lX", offset,
              hdr.offset_nodes + offset);
    ...
    case MD_NODE_HDR:
        ...
        log_debug("\t\t\t * Node values: [flag, objnum, unk1]: %d %d %d",
                  (int)ndhdr.flag, (int)ndhdr.object_number, (int)ndhdr.c_unk1);

        parseSubNode(in, thdr, hdr, hdr2, objidx, shdr,
    ...
    case MD_NODE_SPLIT:
        ...
        else if ((ns.behind_node >=
                  shdr.node_start)) { // todo:  && ( ns.behind_node < NodeMax )
            parseSubNode(in, thdr, hdr, hdr2, objidx, shdr, ns.behind_node);
        }

        if (ns.behind_node < offset)
            log_error("\t\t\tSplit to a lower node number - front node!");
        else if ((ns.front_node >=
                  shdr.node_start)) { // todo: && ( ns.front_node < NodeMax )
    ...
    default:
        log_error("Unknown node type %d at offset %04lX", splittype,
                  hdr.offset_nodes + offset);
    }
}

void LoadTreeNodeGeometry(ifstream &in, BinHeadType &thdr, BinHeader &hdr,
                          BinHeader2 &hdr2, int objidx, SubObjectHeader &shdr) {

    parseSubNode(in, thdr, hdr, hdr2, objidx, shdr, shdr.node_start);
}

/* Processes object list, and should prepare the global vertex buffer, index
 * buffer, skeleton tree, vertex - joint mapping
 */
void ProcessObjects(ifstream &in, BinHeadType &thdr, BinHeader &hdr,
                    BinHeader2 &hdr2) {
    ...
    // TODO: I do not have other choice, than to make the skeleton so the VHOTS
    // in the sub-objects are made as bones from the joint point to the vhot.
    // This is because the ogre does not have anything like the VHOT vertices...
    // (This should not be a serious problem, I hope - they only should have
    // compatible naming - VHOT1 for example)

    for (int x = 0; x < hdr.num_objs; x++) {
        // logging the object names:
        log_debug("!!!    - sub-object name : %s", objects[x].name);
        log_debug("       	- movement type : %d",
                  (int)objects[x].movement);

        // the sub-object readout. We directly process the geometry too, and
        // build the constructs needed to export the file

        // The v. 6 file has a direct list of triangle vertex and uv indices,
        // the others have a BSP structured data we call the add Triangle
        // function on the resulting triangle anyway...
        if (thdr.version == 6) {
    ...
    // the others have a good transformation info, and the connection of
    // skeleton bones seems to be dependent on the sub-object names
}
...
void readObjectModel(ifstream &in, BinHeadType &thdr) {
    ...
    case 3:
        size = SIZE_BIN_HDR_V3;
        break;
    case 4:
        size = SIZE_BIN_HDR_V4;
        break;
    case 6:
        size = SIZE_BIN_HDR_V6;
        break;
    default:
        size = -1; // TODO: fixfix
    }
    ...
    if (size <= 0) {
        log_fatal("FATAL: The object mesh has an unsupported version : %d",
                  thdr.version);
        return;
    }
    ...
    // we should definetaly complete the header data depending on the version
    // somehow...
    ...
    for (int x = 0; x < hdr.num_mats; x++) {
        ...
        log_debug("       - material name : %s (type: %04X, slot: %d)",
                  materials[x].name, materials[x].type, materials[x].slot_num);
        ...
        SingleMaterialMesh *ins = new SingleMaterialMesh(
            fileBaseName, x, (materials[x].type == MD_MAT_TMAP));
    ...
    if (hdr.mat_flags & MD_MAT_TRANS || hdr.mat_flags & MD_MAT_ILLUM) {
        ...
        for (int i = 0; i < hdr.num_mats; ++i) {
            ...
            log_debug("   - Mat. %d trans %f illum %f", i,
                      materialsExtra[i].trans, materialsExtra[i].illum);
        }
    }
    ...
    // I have to rely on shadowspawn here: read uvs in the number of verts -
    // those are differing, but I guess that this is solvable later on After
    // looking into the binary, the num_uvs calculated seem to be reasonable
    // in.read((char *) uvs, hdr.num_verts * sizeof(UVMap));
    in.read((char *)uvs, num_uvs * sizeof(UVMap));

    // TODO: shadowspawn reverses the U part of pre-6 version mesh UV table
    // here. See if we need that too
    ...
    snprintf(filepath, 1024, "%s%s.xml", meshOutPath,
             fileBaseName); // TODO: Path!
...
void displayHelpAndExit() {
    cout << "meshconverter [-x XML-out-path] [-m Material-out-path] [-l "
            "loglevel] filename"
         << endl;
    cout << "Loglevel : 0 - fatal, 1 - error, 2 - info, 3 - debug 4-all"
         << endl;
...
void putTrailingSlash(char **string) {
    if (strlen(*string) > 0) {
        ...
        if (endc != PLATFORM_SLASH) { // no trailing slash!
            ...
            sprintf(newstring, "%s%c", *string,
                    PLATFORM_SLASH); // string allocation - aaw the plain C
...
void parseCmdLine(int argc, char *argv[]) {
    ...
    while (++pos < argc) {
        if (argv[pos][0] == '-') { // an option switch
            ...
            log_debug("Argument %s (%c) is a filename", argv[pos],
                      argv[pos][0]);
            fileName = argv[pos];
        }
        ...
        if (pos >= argc) {
            log_error("Some option has unsatisfied parameters (option without "
                      "value)");
            displayHelpAndExit();
        }
    }
    ...
    // cut - out the base name of the input file, and save it into the
    // filebasename
    char helper[255];
...
int main(int argc, char *argv[]) {
    ...
    } else if (strncmp(header.ID, "LGMM", 4) == 0) {
