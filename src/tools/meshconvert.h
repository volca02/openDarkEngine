...
// what would definetaly be nice would be some << operators for these used structs, this way we could cout << material[1]; One can dream!
...
//	put some vectors including our prepared structs out. (uv vectors, vertices, triangle-list)
//		and optional vertex -> joint mapping (if specified)

/*
The thing is that each vertex in the output list has one Uv mapping entry. So if we encounter a vertex index that allready exists in the list,
we have to look if it has the same uv. (so we have to look for combinations vertex-uv)
*/
using namespace std;
