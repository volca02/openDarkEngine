...
String
    PLDefScriptCompiler::pldefScript_BNF =
        "<Script> ::= {<Script_Properties>} \n"

        "<Script_Properties> ::= <LinkDefinition> | <PropertyDefinition> | "
        "<DChunkVersion> | <LChunkVersion> | <RelChunkVersion> | "
        "<PChunkVersion> \n" // so I can define default version for the chunks

        // Link definition part
        "<LinkDefinition> ::= 'relation' <Label> '{' {<LinkParams>} '}' \n"

        "<LinkParams> ::= <DTypeName> | <FakeSize> | <LinkHidden> | <NoDType> "
        "| <DChunkVersion> | <LChunkVersion> | <DataCache>\n"

        "<DTypeName> ::= 'dtype' <Flex_Label> \n"

        "<NoDType> ::= 'no_data' \n"

        "<FakeSize> ::= 'fake_size' <Integer> \n" // if dark reports a different
                                                  // size, report that one
        ...
        "<PropertyParams> ::= <DTypeName> | <PropLabel> | <PChunkVersion> | "
        "<PInheritor> | <DataCache> | <DataType>\n"
...
void PLDefScriptCompiler::setupTokenDefinitions(void) {
    addLexemeTokenAction("{", ID_OPENBRACE,
                         &PLDefScriptCompiler::parseOpenBrace);
    addLexemeTokenAction("}", ID_CLOSEBRACE,
                         &PLDefScriptCompiler::parseCloseBrace);

    addLexemeTokenAction("relation", ID_RELATION,
                         &PLDefScriptCompiler::parseRelation);
    addLexemeTokenAction("dtype", ID_DTYPE, &PLDefScriptCompiler::parseDType);
    addLexemeTokenAction("fake_size", ID_FAKESIZE,
                         &PLDefScriptCompiler::parseFakeSize);
    addLexemeTokenAction("no_data", ID_NO_DATA,
                         &PLDefScriptCompiler::parseNoData);
    ...
    addLexemeTokenAction("rel_ver", ID_REL_VER,
                         &PLDefScriptCompiler::parseVersion);

    addLexemeTokenAction("property", ID_PROPERTY,
                         &PLDefScriptCompiler::parseProperty);
    ...
    addLexemeTokenAction("inherit", ID_INHERIT,
                         &PLDefScriptCompiler::parseInherit);
...
//-----------------------------------------------------------------------
size_t PLDefScriptCompiler::getAutoTokenIDStart() const { return ID_AUTOTOKEN; }

//-----------------------------------------------------------------------
void PLDefScriptCompiler::addLexemeTokenAction(const String &lexeme,
                                               const size_t token,
                                               const PLDS_Action action) {
...
void PLDefScriptCompiler::parseCloseBrace(void) {
    if (mCurrentState.state == CS_RELATION) {
        ...
        if (!mCurrentState.no_data) // if data definition is requested
            dt = getTypeDef(
                "links",
                mCurrentState.dtypename); // TODO: Hardcoded. A problem?
        ...
        RelationPtr rel = mLinkService->createRelation(mCurrentState.name, stor,
                                                       mCurrentState.hidden);

        if (mCurrentState.fake_size >= 0)
            rel->setFakeSize(mCurrentState.fake_size);

        rel->setChunkVersions(mCurrentState.lmajor, mCurrentState.lminor,
                              mCurrentState.dmajor, mCurrentState.dminor);
    } else if (mCurrentState.state == CS_PROPERTY) {
        if (mCurrentState.label ==
            "") // default to property chunk name silently
        ...
        if (mCurrentState.ptype == "varstr") {
            ...
            stor = DataStoragePtr(
                new StructuredDataStorage(dt, mCurrentState.cached));
        }

        pg = mPropertyService->createProperty(mCurrentState.label,
                                              mCurrentState.name,
                                              mCurrentState.inherit, stor);
...
//-----------------------------------------------------------------------
DTypeDefPtr PLDefScriptCompiler::getTypeDef(const std::string &group,
...
//-----------------------------------------------------------------------
void PLDefScriptCompiler::parseHidden(void) { mCurrentState.hidden = false; }

//-----------------------------------------------------------------------
void PLDefScriptCompiler::parseNoData(void) { mCurrentState.no_data = true; }

//-----------------------------------------------------------------------
void PLDefScriptCompiler::parseCached(void) { mCurrentState.cached = true; }
...
void PLDefScriptCompiler::parseVersion(void) {
    ...
    if (mCurrentState.state == CS_RELATION ||
        ...
        if (vid == ID_D_VER) {
            ...
            logParseError("Only data/link/property versions are allowed while "
                          "in property/relation definition");
        }
