...
/** DType (Dynamic typedef) script compiler. Compiles the C-like syntax
(sort-of) from the given DataStream, and fills the BinaryService, with the
...
<br>
Type [use Enum/Bitfield] name [ [array size] ] name [ = "default value string"
]<br> <br> <B>in case of type == char (fixed string), a box brace follows the
char like this:</B><br> char [64] a_long_string;<br> <br> <B>Type is one of:
<i>uintXX, intXX, float, boolXX, varstr, vector, char [len]</i></B> <br> <br>
<B>varstr (variable length string) cannot be contained in a
struct/union/array</B>
</code>
*/
...
public:
     ...
     */
    const Ogre::String &getClientBNFGrammer(void) const {
        return dtypeScript_BNF;
    }

    /** get the name of the BNF grammar.
     */
    virtual const Ogre::String &getClientGrammerName(void) const {
        static const Ogre::String grammerName("DType Script");
        return grammerName;
    }
    ...
    /** Returns the position usable for autogenerated tokens to use - ogre's
     * requirement */
...
protected:
    ...
    /** Compilation state. The position the compilation is in, together with
    ...
    /** The stack of the current position (Namespace->struct->struct for
     * example) */
    ...
    /** Push the current state to the state stack top, and set the current to
     * CS_UNKNOWN */
    ...
    /** Convert to DVariant::Type from TokenID, or logParseError if
     * unsuccessfull */
    ...
    /** get the typedef from binary service. First the namespace's, then the
     * global if not found in namespace */
    DTypeDefPtr getTypeDef(const std::string &name);

    /** get the enumeration from binary service. First the namespace's, then the
     * global if not found in namespace */
    ...
    /** Expects, and reads the number of elements in the [] parenthesis. Returns
     * the parsed number */
    int parseBoxBrace();
    ...
    void addLexemeTokenAction(const Ogre::String &lexeme, const size_t token,
                              const DTS_Action action = 0);
