...
// Inspired by Ogre's material compiler, parts of the code were taken from the
// Ogre projects
...
String DTypeScriptCompiler::dtypeScript_BNF =
    ...
    "<Enum> ::= 'enum' <Type_Label> [<Space_Character>] ':' <Variant_Type> '{' "
    "<Enum_Keys> '}' \n"
    ...
    // Alias type to another name : alias type new_name [optional array_len]
    // (searches actual namespace then global namespace)
    "<Alias> ::= 'alias' <Type_Label> [<Space_Character>] <Type_Label> "
    "[<Array_Spec>] \n"

    "<Struct_Header> ::= 'struct' | 'union' \n"

    // Struct definition
    "<Struct> ::= <Struct_Header> <Type_Label> [<Array_Spec>] '{' "
    "{<Struct_Fields>} '}' \n"
    ...
    // Example: int32 use myEnum TenEnums [10] = 0 //(this will create array of
    // 10 fields of int32, enumerated, with default value 0) Example: char[10]
    // TenStrings [10] = "Hello" //(this will create array of 10 strings with
    // default value "Hello")
    "<Field> ::= <Type_Definition> [<Enum_Ref>] <Type_Label> [<Array_Spec>] "
    "[<Default_Value>] \n"
    ...
    "<Primitive_Type> ::= 'int32' | 'uint32' | 'int16' | 'uint16' | 'int8' | "
    "'uint8' | 'float' | 'vector' | 'shortvec' | 'bool32' | 'bool16' | 'bool8' "
    "\n"

    "<Variant_Type> ::= 'uint' | 'int' | 'float' | 'bool' | 'string' | "
    "'vector' \n"
    ...
    "<Type_Label_legals_Ext> ::= "
    "(abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-0123456789) \n"

    "<Integer> ::= (0123456789) { (0123456789) } \n"

    "<Unquoted_Label> ::= <Unquoted_Label_Illegals> "
...
DTypeScriptCompiler::DTypeScriptCompiler(void)
    : mStateStack(), mBinaryService(NULL) {
...
void DTypeScriptCompiler::setupTokenDefinitions(void) {
    addLexemeTokenAction("{", ID_OPENBRACE,
                         &DTypeScriptCompiler::parseOpenBrace);
    addLexemeTokenAction("}", ID_CLOSEBRACE,
                         &DTypeScriptCompiler::parseCloseBrace);

    addLexemeTokenAction("namespace", ID_NAMESPACE,
                         &DTypeScriptCompiler::parseNameSpace);

    addLexemeTokenAction("enum", ID_ENUM, &DTypeScriptCompiler::parseEnum);
    addLexemeTokenAction("bitfield", ID_BITFIELD,
                         &DTypeScriptCompiler::parseEnum);
    ...
    addLexemeTokenAction("struct", ID_STRUCT,
                         &DTypeScriptCompiler::parseStruct);
    ...
    addLexemeTokenAction("shortvec", ID_SHORTVECTOR,
                         &DTypeScriptCompiler::parseField);

    addLexemeTokenAction("char", ID_CHAR, &DTypeScriptCompiler::parseField);
}

//-----------------------------------------------------------------------
size_t DTypeScriptCompiler::getAutoTokenIDStart() const { return ID_AUTOTOKEN; }

//-----------------------------------------------------------------------
void DTypeScriptCompiler::addLexemeTokenAction(const String &lexeme,
                                               const size_t token,
                                               const DTS_Action action) {
...
void DTypeScriptCompiler::executeTokenAction(const size_t tokenID) {
    ...
    if (action == mTokenActionMap.end()) {
        ...
        try {
            ...
            // in finding a valid terminal token to complete the rule
            // expression.
...
void DTypeScriptCompiler::logParseError(const String &error) {
    // LOG_ERROR("Error in script at line %d  of %s (last context: %s) : %s",
    // mCurrentLine, mSourceName.c_str(), mCurrentState.name.c_str(),
    // error.c_str());

    OGRE_EXCEPT(Ogre::Exception::ERR_INTERNAL_ERROR,
                "Error in script at line " +
                    StringConverter::toString(mCurrentLine) + " of " +
                    mSourceName + ": " + error,
                "DTypeScriptCompiler::logParseError");
}
...
void DTypeScriptCompiler::parseCloseBrace(void) {
    ...
    if (was.state == CS_ENUM) { // Closing the enum/bitfield definition
        if (mCurrentState.state == CS_STRUCT) {
            logParseError("Enum cannot be created inside structs, use "
                          "pre-defined enums only.");
        }

        if (mCurrentState.state == CS_ENUM) {
            logParseError("Enum inside enum error - found enum " +
                          mCurrentState.name + " when closing " + was.name);
        }
    ...
    if (was.state ==
        CS_NAMESPACE) // closing namespace, erase the namespace's name
        mGroupName = "";
}
...
void DTypeScriptCompiler::parseNameSpace(void) {
    ...
    // Namespace does not push itself to the stack. it always is at the lowest
    // position. Pushing happens while creating containers
    const String newGrp = getNextTokenLabel();

    if (mGroupName != "" || mCurrentState.state == CS_NAMESPACE)
        logParseError("Nesting namespaces is not possible - " + newGrp +
                      " inside " + mGroupName);

    if (mCurrentState.state != CS_UNKNOWN)
        logParseError("Namespace inside a declaration is not possible : " +
                      newGrp);
...
DVariant::Type DTypeScriptCompiler::getDVTypeFromStr(const std::string &name) {
    ...
    } else if (name == "uint" || name == "uint32" || name == "uint16" ||
               name == "uint8") {
        return DVariant::DV_UINT;
    } else if (name == "bool" || name == "bool32" || name == "bool16" ||
               name == "bool8") {
    ...
    OPDE_EXCEPT("Unknown type specified : " + name,
                "DTypeScriptCompiler::getDVTypeFromStr");
}
...
void DTypeScriptCompiler::dispatchType(DTypeDefPtr &def) {
    if (mCurrentState.state == CS_STRUCT) {
        ...
        mCurrentState.types.push_back(
            def); // will get released on closing brace
...
void DTypeScriptCompiler::parseEnum(void) {
    ...
    if (!bitfield) {
        ...
        if (paramCount != 2)
            logParseError("Type specifier for enumeration '" +
                          mCurrentState.name + "' expected");
        ...
        mCurrentState.enumeration =
            DEnumPtr(new DEnum(mCurrentState.name, ntype, false));
    } else {
        mCurrentState.enumvaltype = DVariant::DV_UINT;
        mCurrentState.enumeration =
            DEnumPtr(new DEnum(mCurrentState.name, DVariant::DV_UINT, true));
    }
}
...
void DTypeScriptCompiler::parseField(void) {
    ...
    // Look at the next token. it can either be 'use', array spec '[' or direct
    // name of the field
    ...
    if (testNextTokenID(ID_EQUALS)) {
        ...
        try {
            ...
            logParseError("Exception while constructing the default field's '" +
                          name + "' value");
        }
    }
    ...
    if (hasDefault)
        try {
            ...
            logParseError("Exception while constructing the field's '" + name +
                          "' type : " + ex.getDetails());
        }
    else
        try {
...
int DTypeScriptCompiler::parseBoxBrace() {
    ...
    try {
        ...
        array_len = 0;
        logParseError("Exception parsing the array len : " +
                      ex.getDescription());
    }
