...
ProxyArchive::ProxyArchive(const String &name, const String &archType,
                           bool readOnly)
    : Archive(name, archType), mArchive(NULL) {}
...
// -------------------------------------------------------
FileInfoListPtr ProxyArchive::findFileInfo(const String &pattern,
...
// -------------------------------------------------------
StringVectorPtr ProxyArchive::find(const String &pattern, bool recursive,
                                   bool dirs) const {
    /// have to list all infos, filter those which fit
...
// -------------------------------------------------------
FileInfoListPtr ProxyArchive::listFileInfoImpl(bool recursive,
                                               bool dirs) const {
...
bool ProxyArchive::match(const String &pattern, const String &name) const {
    ...
    // inspired by one codeproject article (but a rewrite without using the
    // code)

    enum State { PM_Match = 0, PM_Any, PM_AnySeq };
    ...
    while (match && pi != lpattern.end()) {
        ...
        case PM_AnySeq:
            ...
            if (pnext != lpattern.end() && si == unt.end()) {
                match = false;
                break;
            }
...
// -------------------------------------------------------
CaseLessFileSystemArchive::~CaseLessFileSystemArchive(void) { delete mArchive; }

// -------------------------------------------------------
bool CaseLessFileSystemArchive::isCaseSensitive(void) const { return false; }
...
// ----- CRF Archive -------------------------------------
// -------------------------------------------------------
CRFArchive::CRFArchive(const String &name, const String &archType,
                       bool readOnly)
    : ProxyArchive(name, archType, readOnly) {
...
// -------------------------------------------------------
CRFArchive::~CRFArchive(void) { delete mArchive; }

// -------------------------------------------------------
bool CRFArchive::isCaseSensitive(void) const { return false; }
