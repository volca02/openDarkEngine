 ...
 *    Based on Thief Font Converter by Tom N Harris
...
char **ReadFont(FILE *FontFilePointer, int *ResultingWidth,
                int *ResultingHeight, int *ResultingColor,
                const char *FontName) {
    ...
    for (N = 0; N < NumChars; N++) {
        if (FontHeader.FirstChar + N == ' ')
            sprintf(FontDefData, "\t\t//glyph %c %f %f %f %f\n",
                    FontHeader.FirstChar + N,
                    (float)(Chars[N].X - 2) / FinalSize,
                    (float)(Chars[N].Y - 2) / FinalSize,
                    (float)(Chars[N].X + Chars[N].Width + 2) / FinalSize,
                    (float)(Chars[N].Y + FontHeader.NumRows + 2) /
                        FinalSize); // Ogre doesn't like the space character
        else
            sprintf(FontDefData, "\t\tglyph %c %f %f %f %f\n",
                    FontHeader.FirstChar + N,
                    (float)(Chars[N].X - 2) / FinalSize,
                    (float)(Chars[N].Y - 2) / FinalSize,
                    (float)(Chars[N].X + Chars[N].Width + 2) / FinalSize,
                    (float)(Chars[N].Y + FontHeader.NumRows + 2) / FinalSize);
        fwrite(FontDefData, strlen(FontDefData), 1, FilePointer);
    }
    ...
    if (FontHeader.Format == 0) {
        ...
        for (I = 0; I < FontHeader.NumRows; I++) {
            for (N = 0; N < NumChars; N++) {
                ...
                for (X = 0; X < Chars[N].Width; Y++, X++)
                    RowPointers[Chars[N].Y + I][Chars[N].X + X] =
        ...
        for (I = 0; I < FontHeader.NumRows; I++) {
            for (N = 0; N < NumChars; N++) {
                memcpy(RowPointers[Chars[N].Y + I] + Chars[N].X, Ptr,
                       Chars[N].Width);
                Ptr += Chars[N].Width;
            }
        }
...
int WriteImage(FILE *FilePointer, RGBQUAD *ColorTable, char **RowPointers,
...
RGBQUAD *ReadPalette(FILE *FilePointer) {
    ...
    if (PaletteHeader.RiffSig == 0x46464952) {
        ...
        Bytes = fread(P, sizeof(RGBTRIPLE) * 256, 1, FilePointer) *
                sizeof(RGBTRIPLE) * 256;
...
int FontToImage(const char *FontName, const char *PaletteFile) {
    ...
    ImageRows =
        ReadFont(FilePointer, &ImageWidth, &ImageHeight, &Color, FontName);
    fclose(FilePointer);
    if (!ImageRows) {
        if (PaletteData != ColorTable)
            free(PaletteData);
    ...
    Error = WriteImage(FilePointer, (RGBQUAD *)PaletteData, ImageRows,
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        ...
        fprintf(stdout, "Based on Thief Font Converter by Tom N Harris "
                        "<telliamed@whoopdedo.cjb.net>\n\n");
        fprintf(stdout,
                "Usage: DarkFontConverter FontFileName [PaletteFile]\n");
        return 1;
    }
    Font = argv[1];
