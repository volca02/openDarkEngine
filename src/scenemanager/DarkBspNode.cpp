...
Rewritten to use in the openDarkEngine project by Filip Volejnik
...
//-----------------------------------------------------------------------
BspNode::BspNode()
    : mOwner(NULL), mIsLeaf(false), mSceneNode(NULL), mFront(NULL),
      mBack(NULL) {
    // update the fragment as wellBspNode
    mCellFragment.fragmentType = SceneQuery::WFT_CUSTOM_GEOMETRY;
    mCellFragment.geometry = this;
...
//-----------------------------------------------------------------------
void BspNode::_addMovable(const MovableObject *mov) { mMovables.insert(mov); }

//-----------------------------------------------------------------------
void BspNode::_removeMovable(const MovableObject *mov) { mMovables.erase(mov); }
...
//-----------------------------------------------------------------------
void BspNode::setIsLeaf(bool isLeaf) { mIsLeaf = isLeaf; }
...
//-----------------------------------------------------------------------
void BspNode::setOwner(SceneManager *owner) { mOwner = owner; }
...
//-------------------------------------------------------------------------
unsigned int BspNode::getCellNum() const { return mCellNum; }

//-------------------------------------------------------------------------
void BspNode::setPlaneList(BspNode::CellPlaneList &planes,
                           BspNode::PlanePortalMap &portalmap) {
    mPlaneList = planes;
...
//-------------------------------------------------------------------------
bool BspNode::isVisBlocked() { return (mCellFlags & CELL_VIS_BLOCK) != 0; }

//-------------------------------------------------------------------------
void BspNode::_setCellFlags(unsigned int flags) { mCellFlags = flags; }

//-------------------------------------------------------------------------
void BspNode::testAndSetDistributed(unsigned int prereq, unsigned int mask,
                                    unsigned int addition) {
