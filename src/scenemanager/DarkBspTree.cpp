...
Rewritten from the BSPLevel class to use in the openDarkEngine project by Filip
Volejnik <f.volejnik@centrum.cz>
...
//-----------------------------------------------------------------------
BspTree::~BspTree() { clear(); }

//-----------------------------------------------------------------------
const BspNode *BspTree::getRootNode(void) { return mRootNode; }

//-----------------------------------------------------------------------
void BspTree::setRootNode(BspNode *node) { mRootNode = node; }

//-----------------------------------------------------------------------
void BspTree::setRootNode(int id) { mRootNode = getNode(id); }
...
//-----------------------------------------------------------------------
void BspTree::findLeafsForSphere(BspNodeList &destList, const Vector3 &pos,
                                 Real radius) {
    findLeafsForSphereFromNode(mRootNode, destList, pos, radius);
}

//-----------------------------------------------------------------------
void BspTree::findLeafsForSphereFromNode(BspNode *node, BspNodeList &destList,
...
const LightList *BspTree::objectQueryLights(const MovableObject *movable) {
    ...
    if (it != mMovableLightsCache.end()) {
        ...
        // no entry found. Build the list of lights, cache
        std::pair<MovableLightListCache::iterator, bool> r =
            mMovableLightsCache.insert(std::make_pair(movable, LightList()));
