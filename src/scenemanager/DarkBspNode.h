...
Rewritten to be used in openDarkEngine project by Filip Volejnik
...
/** Encapsulates a node in a BSP tree.
    ...
    partitioned is either the world (in the case of the root node) or the space
   derived from their parent node. Each node can have elements which are in
   front or behind it, which are it's children and these elements can either be
   further subdivided by planes, or they can be undivided spaces or 'leaf nodes'
   - these are the nodes which actually contain objects and world geometry. The
   leaves of the tree are the stopping point of any tree walking algorithm, both
   for rendering and collision detection etc.</p> Ogre chooses not to represent
   splitting nodes and leaves as separate structures, but to merge the two for
   simplicity of the walking algorithm. If a node is a leaf, the isLeaf() method
   returns true and both getFront() and getBack() return null pointers. If the
   node is a partitioning plane isLeaf() returns false and getFront() and
   getBack() will return the corresponding BspNode objects.

    This version of BspNode, updated to be used by DarkSceneManager, implements
   Portals as members. It should be considered as a 'Cell' if it is a leaf.
*/
class BspNode {
...
public:
    ...
    /** Returns true if this node is a leaf (i.e. contains geometry) or false if
       it is a splitting plane. A BspNode can either be a splitting plane (the
       typical representation of a BSP node) or an undivided region contining
       geometry (a leaf node). Ogre represents both using the same class for
       simplicity of tree walking. However it is important that you use this
       method to determine which type you are dealing with, since certain
       methods are only supported with one of the subtypes. Details are given in
       the individual methods. Note that I could have represented splitting /
       leaf nodes as a class hierarchy but the virtual methods / run-time type
       identification would have a performance hit, and it would not make the
        code much (any?) simpler anyway. I think this is a fair trade-off in
       this case.
    */
    inline bool isLeaf(void) const { return mIsLeaf; };

    /** Returns a pointer to a BspNode containing the subspace on the positive
       side of the splitting plane. This method should only be called on a
       splitting node, i.e. where isLeaf() returns false. Calling this method on
       a leaf node will throw an exception.
    */
    inline BspNode *getFront(void) const {
        assert(!mIsLeaf);
        return mFront;
    };

    /** Returns a pointer to a BspNode containing the subspace on the negative
       side of the splitting plane. This method should only be called on a
       splitting node, i.e. where isLeaf() returns false. Calling this method on
       a leaf node will throw an exception.
    */
    ...
    /** Determines which side of the splitting plane a worldspace point is.
        This method should only be called on a splitting node, i.e. where
       isLeaf() returns false. Calling this method on a leaf node will throw an
       exception.
    */
    ...
    */
    ...
    /** Returns details of the plane which is used to subdivide the space of his
       node's children. This method should only be called on a splitting node,
       i.e. where isLeaf() returns false. Calling this method on a leaf node
       will throw an exception.
    */
    const Plane &getSplitPlane(void) const;

    /** Returns the axis-aligned box which contains this node if it is a leaf.
        This method should only be called on a leaf node. It returns a box which
       can be used in calls like Camera::isVisible to determine if the leaf node
    ...
    */
    ...
    /// Internal method for telling the node that a movable no longer intersects
    /// it
    ...
    /** Returns a reference to the scene node representing this leaf. Throws
     * exception on the non-leaf nodes. */
    SceneNode *getSceneNode();

    /** Sets the associated scene node. That is the scene node represented by
     * this leaf */
    void setSceneNode(SceneNode *node);

    // BspNode parameter settings. As we want to construct the BspNodes
    // Externally...
    ...
    /** Internal routine for flag settings - only to be called when initializing
     * the cell */
...
protected:
     ...
     * b) if not, it will be set, and all portal connected targets will be
     * called with the same parameters
     *
     * @param prereq The prerequisite for the test to happen - the and operation
     * of the cell's flags with this must be nonzero
     * @param mask The mask to apply - cell flags will be and-ed with this
     * parameter
     * @param addition The additional bits to set - cell flags will be or-ed
     * with this parameter after the prev. masking
     */
    void testAndSetDistributed(unsigned int prereq, unsigned int mask,
    ...
    /// Owner Scene Manager - Back-reference to SceneManager which owns this
    /// Node
    SceneManager *mOwner;
    ...
    /** The plane which splits space in a non-leaf node.
        Note that nodes do not allocate the memory for other nodes - for
       simplicity and bulk-allocation of memory the BspLevel is responsible for
       assigning enough memory for all nodes in one go.
    */
    Plane mSplitPlane;
    ...
    /** The set of movables that could be visible in this BSP node, and thus
     * have to be rendered when this node is visible */
    ...
    // For acceleration, we prepare a world fragment too
    // (WFT_PLANE_BOUNDED_REGION)
    /** World fragment if someone wants the cell as a result from the query. - a
     * pre-prepared fragment containing the cell */
