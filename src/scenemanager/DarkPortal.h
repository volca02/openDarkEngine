 ...
 *
 * This structure is used by Portal class when calculating the on-screen
...
struct OPDELIB_EXPORT PortalRect {
    ...
    /// intersects this and b rects, and returns true if intersection produced
    /// non-empty rect
    bool intersect(const PortalRect &b, PortalRect &result) const {
        ...
        // distance has to stay ours, otherwise the dist would propagate zero
        // everywhere
    ...
    void enlargeToContain(const Vector3 pnt) {
        ...
        // NOTE: The X coord is flipped. This is because of the transform
        // matrix, but it is not a problem for us
...
struct CellRectInfo {
    CellRectInfo() : updateID(0), rect(PortalRect::EMPTY) {}
...
struct PortalRectInfo {
    PortalRectInfo()
        : portalCull(false), screenRect(PortalRect::EMPTY),
    ...
    /**
     * Intersects the Portals bounding rectangle by the given rectangle, and
     * writes the result to the target parameter
     */
    ...
    /**
     * Union the actual view rectangle with addition rectangle. Returns true on
     * a view change.
     */
...
public:
    ...
    CellRectInfo &cell(unsigned int id) { return cellRects[id]; }

    PortalRectInfo &portal(unsigned int id) { return portalRects[id]; }
 ...
 * visibility determination is done using a to-screen projected portal vertices
 * bounding rectangles PortalRects
...
public:
    /** Default constructor. Defines an empty geometry, and source and
     * destination SceneNodes that are parameters
     ...
     * @param toScreen A precomputed Projection*View matrix matrix4 which is
     * used to project the vertices to screen
     * @param cutp Plane to be used to cut the portal (cameras near plane, or
