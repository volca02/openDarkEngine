...
public:
    /** Default constructor. Defines an empty geometry which will lie on the
     * specified plane
     ...
     * @return the count of points which lie outside (negative distance from the
     * plane)
     */
    unsigned int getOutCount(const Plane &plane);

    /** Clips the poly using a plane (and replaces instances vertices after
     * success)
     * @param plane The plane to clip with
     * @param didClip is set to true if the polygon was changed by the clipping
     * operation
     * @return number of vertices in the new poly
     */
    int clipByPlane(const Plane &plane, bool &didClip);

    /** Optimizes the portal. Removes unneeded, unnecessary vertices which slow
     * down the visibility evaluation.
    ...
    /** Returns true if the sphere is totaly enclosed by the polygon on the
     * intersection with the portal's plane
     * @param pos Sphere center
     * @param radius Sphere radius
     * @param distance Sphere center to polygon's plane distance
     */
    bool enclosesSphere(const Vector3 &pos, const Real &radius,
