...
void LightService::_loadTableFromTagFile(const FilePtr &tag) {
    ...
    for (; it != mLightsForCell.end(); ++it) {
        ...
        BspNode *leaf =
            static_cast<DarkSceneManager *>(mSceneMgr)->getBspLeaf(it->first);
...
LightsForCell::LightsForCell(const FilePtr &file, size_t num_anim_lights,
    ...
    : mLightSize(light_size), mNumTextured(num_textured),
      mNumAnimLights(num_anim_lights), mFaceInfos(face_infos), mAtlased(false) {
