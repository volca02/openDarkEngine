...
MaterialInstance::~MaterialInstance() { clearCopyMaterial(); }

void MaterialInstance::setSceneBlending(SceneBlendType sbt) {
    mSBT = sbt;

    if (mCopyMat) {
        Material::TechniqueIterator techniqueIt =
            mCopyMat->getTechniqueIterator();
        ...
        while (techniqueIt.hasMoreElements()) {
            ...
            while (passIt.hasMoreElements()) {
                ...
                default:
                    ...
                    passIt.peekNext()->setAmbient(mOriginalMat->getTechnique(i)
                                                      ->getPass(j)
                                                      ->getAmbient());
                    break;
                }
...
MaterialPtr MaterialInstance::getCopyMaterial() { return mCopyMat; }

void MaterialInstance::createCopyMaterial() {
    String name;
    // Avoid name collision
    do {
        name = mOriginalMat->getName() +
               StringConverter::toString(Math::UnitRandom());
