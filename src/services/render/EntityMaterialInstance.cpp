...
Ogre::uint8 EntityMaterialInstance::msBaseRenderQueueGroup =
    127; // Must be set!
...
EntityMaterialInstance::~EntityMaterialInstance() { destroySEMIs(); }
...
EntityMaterialInstance::getSubEntityMaterialInstancesIterator() {
...
void EntityMaterialInstance::prepareSEMIs() {
    ...
    for (unsigned int i = 0; i < mEntity->getNumSubEntities(); i++) {
        mSEMIs.push_back(
            new SubEntityMaterialInstance(mEntity->getSubEntity(i)));
    }
}
