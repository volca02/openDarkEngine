 ...
 * an utility to allow a single Renderable to change its transparency without
 * changing every other renderables' transparency which also use this material.
 * It will create (and use) a copy of the original material only when needed.
 * But it will keep the reference to the cloned material until it's no longer
 * needed (to save re-clonation time). Allows also changing the Renderable
 * material through this class, so it's no need to destroy this instance and
 * create a new one if the material changes.
 * @note Transparency is applied to all the passes in all the techniques.
 * @note Default blending method is alpha blending. Blending methods in the
 * original material will be overriden.
 * @note Modulative blending is not supported as it can't be done if some
 * textures exist.
 * @author Kencho
 * @todo Check lighting enabled or not. Disabled lighting won't allow colour
 * changing.
 * @todo Take care of shininess, specularity, and emissiveness.
 * @todo Add existing material recognising support (to allow existing
...
protected:
    ...
    /** Returns true if this material instance has any settings different from
     * default
     */
