...
BinaryService::BinaryService(ServiceManager *manager, const std::string &name)
    : ServiceImpl<BinaryService>(manager, name), mTypeGroups(), mEnumGroups() {
    Opde::ConsoleBackend::getSingleton().registerCommandListener(
        "dtdesc", dynamic_cast<ConsoleCommandListener *>(this));
    Opde::ConsoleBackend::getSingleton().setCommandHint(
        "dtdesc", "Describe a dynamic type (param : dtype path)");
};

//------------------------------------
BinaryService::~BinaryService() { clearAll(); };

//------------------------------------
bool BinaryService::init() { return true; }

//------------------------------------
void BinaryService::addType(const std::string &group, const DTypeDefPtr &def) {
    LOG_DEBUG("BinaryService: Inserting type %s/%s", group.c_str(),
              def->name().c_str());

    std::pair<TypeGroups::iterator, bool> grp =
        mTypeGroups.insert(TypeGroups::value_type(group, TypeMap()));

    grp.first->second.insert(TypeMap::value_type(def->name(), def));
}

//------------------------------------
void BinaryService::addEnum(const std::string &group, const std::string &name,
                            const DEnumPtr &enm) {
    LOG_DEBUG("BinaryService: Inserting enum %s/%s", group.c_str(),
              name.c_str());

    std::pair<EnumGroups::iterator, bool> grp =
        mEnumGroups.insert(EnumGroups::value_type(group, EnumMap()));
...
//------------------------------------
DTypeDefPtr BinaryService::getType(const std::string &group,
    ...
    if (git != mTypeGroups.end()) {
        ...
        if (it != git->second.end()) {
            ...
            OPDE_EXCEPT(string("Could not find type ") + name + " in group " +
                            group,
                        "BinaryService::getType");
    } else
        OPDE_EXCEPT(string("Could not find group ") + group,
                    "BinaryService::getType");
}
...
//------------------------------------
DEnumPtr BinaryService::getEnum(const std::string &group,
                                const std::string &name) const {
    // find the group
    ...
    if (git != mEnumGroups.end()) {
        ...
        if (it != git->second.end()) {
            ...
            OPDE_EXCEPT(string("Could not find enum ") + name + " in group " +
                            group,
                        "BinaryService::getEnum");
    } else
        OPDE_EXCEPT(string("Could not find group ") + group,
                    "BinaryService::getEnum");
}
...
BinaryService::splitPath(const std::string &path) const {
    ...
    if (sep_pos != string::npos) {
        ...
        if (name_part.find(BINARY_GROUP_SEPARATOR) != string::npos)
            OPDE_EXCEPT("More than one separator found in path",
                        "BinaryService::splitPath");
    }

    return std::pair<std::string, std::string>(group_part, name_part);
}

void BinaryService::commandExecuted(std::string command,
    ...
    if (command == "dtdesc") {
        ...
        // describe the type. Iterate through the fields, write field name,
        // size, type
        for (; it != type->end(); it++) {
            LOG_INFO("[%d] %s (%d)", it->offset, it->name.c_str(),
                     it->type->size());
        }
    } else
        LOG_ERROR("Command %s not understood by BinaryService",
                  command.c_str());
}
...
const std::string &BinaryServiceFactory::getName() { return mName; }

const uint BinaryServiceFactory::getMask() { return SERVICE_CORE; }

const size_t BinaryServiceFactory::getSID() { return BinaryService::SID; }
